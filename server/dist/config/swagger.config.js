...
const swaggerSpec = {
    ...
    tags: [
        ...
        {
            name: "Daily Check-In",
            description: "Daily health check-in — all 5 steps submitted in one request",
        },
        {
            name: "Doctors",
    ...
    paths: {
        ...
        "/api/v1/users/refresh-token": {
            post: {
                tags: ["Users"],
                summary: "Get a new access token (reads refreshToken from httpOnly cookie automatically)",
                security: [],
                responses: {
                    "200": {
        ...
        "/api/v1/doctors": {
            get: {
                tags: ["Doctors"],
                summary: "Get all doctors (optional filters: state, city, specialty, isAvailable)",
                security: [],
                parameters: [
                    {
                        name: "state",
        ...
        "/api/v1/doctors/nearby": {
            get: {
                tags: ["Doctors"],
                summary: "Find available doctors near you (based on your profile state/city)",
                description: "Returns doctors in your city first, then falls back to your state if no city-level matches are found. Requires your profile to have a `state` set.",
                security: [{ bearerAuth: [] }],
                parameters: [
                    {
                        name: "specialty",
